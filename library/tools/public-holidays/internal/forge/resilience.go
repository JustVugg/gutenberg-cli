...
//   PUBLIC_HOLIDAYS_RATE_LIMIT_RPS   tokens per second (0 disables; default 0)
//   PUBLIC_HOLIDAYS_RATE_BURST       burst size (default rps or 1)
//   PUBLIC_HOLIDAYS_RETRY_MAX        max attempts including the first (default 3)
//   PUBLIC_HOLIDAYS_RETRY_BASE_MS    base backoff in ms (default 200)
//   PUBLIC_HOLIDAYS_CB_THRESHOLD     consecutive failures to open the breaker (default 5)
//   PUBLIC_HOLIDAYS_CB_COOLDOWN_MS   cooldown when open (default 30000)
...
func (c *circuitBreaker) Allow() error {
	...
	if c.failures >= c.threshold {
		if time.Since(c.openedAt) < c.cooldown {
			return errors.New("public-holidays: circuit breaker open")
		}
		c.failures = 0
	}
	return nil
...
func initResilience() {
	rps := envFloat("PUBLIC_HOLIDAYS_RATE_LIMIT_RPS", 0)
	burst := envFloat("PUBLIC_HOLIDAYS_RATE_BURST", rps)
	if burst <= 0 {
		burst = 1
	}
	resilienceBucket = newTokenBucket(rps, burst)
	resilienceMax = envInt("PUBLIC_HOLIDAYS_RETRY_MAX", 3)
	if resilienceMax < 1 {
		resilienceMax = 1
	}
	resilienceBaseMs = envInt("PUBLIC_HOLIDAYS_RETRY_BASE_MS", 200)
	cbThreshold := envInt("PUBLIC_HOLIDAYS_CB_THRESHOLD", 5)
	cbCooldownMs := envInt("PUBLIC_HOLIDAYS_CB_COOLDOWN_MS", 30000)
...
func DoWithResilience(req *http.Request) (*http.Response, error) {
	...
	if lastErr == nil {
		lastErr = errors.New("public-holidays: request failed after retries")
	}
	return nil, lastErr
}

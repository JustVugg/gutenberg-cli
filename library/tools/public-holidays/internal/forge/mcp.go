...
func handleRPC(message rpcMessage) (map[string]any, error) {
	...
	case "initialize":
		return map[string]any{"jsonrpc": "2.0", "id": message.ID, "result": map[string]any{
			...
			"serverInfo": map[string]any{"name": "public-holidays-mcp", "version": "0.1.0"},
		}}, nil
	case "notifications/initialized":
		return nil, nil
	case "tools/list":
		return map[string]any{"jsonrpc": "2.0", "id": message.ID, "result": map[string]any{"tools": []map[string]any{
			{"name": "public-holidays_operations", "description": "List operations", "inputSchema": map[string]any{"type": "object", "properties": map[string]any{}}},
			{"name": "public-holidays_call", "description": "Call an operation", "inputSchema": map[string]any{"type": "object", "required": []string{"operationId"}, "properties": map[string]any{"operationId": map[string]any{"type": "string"}, "params": map[string]any{"type": "object"}, "path": map[string]any{"type": "object"}, "yes": map[string]any{"type": "boolean"}}}},
			{"name": "public-holidays_search_cache", "description": "Search cache", "inputSchema": map[string]any{"type": "object", "required": []string{"query"}, "properties": map[string]any{"query": map[string]any{"type": "string"}}}},
		}}}, nil
	case "tools/call":
		name, _ := message.Params["name"].(string)
		args, _ := message.Params["arguments"].(map[string]any)
		if name == "public-holidays_operations" {
			return toolResult(message.ID, Operations()), nil
		}
		if name == "public-holidays_search_cache" {

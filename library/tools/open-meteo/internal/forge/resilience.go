...
//   OPEN_METEO_RATE_LIMIT_RPS   tokens per second (0 disables; default 0)
//   OPEN_METEO_RATE_BURST       burst size (default rps or 1)
//   OPEN_METEO_RETRY_MAX        max attempts including the first (default 3)
//   OPEN_METEO_RETRY_BASE_MS    base backoff in ms (default 200)
//   OPEN_METEO_CB_THRESHOLD     consecutive failures to open the breaker (default 5)
//   OPEN_METEO_CB_COOLDOWN_MS   cooldown when open (default 30000)
...
func (c *circuitBreaker) Allow() error {
	...
	if c.failures >= c.threshold {
		if time.Since(c.openedAt) < c.cooldown {
			return errors.New("open-meteo: circuit breaker open")
		}
		c.failures = 0
	}
	return nil
...
func initResilience() {
	rps := envFloat("OPEN_METEO_RATE_LIMIT_RPS", 0)
	burst := envFloat("OPEN_METEO_RATE_BURST", rps)
	if burst <= 0 {
		burst = 1
	}
	resilienceBucket = newTokenBucket(rps, burst)
	resilienceMax = envInt("OPEN_METEO_RETRY_MAX", 3)
	if resilienceMax < 1 {
		resilienceMax = 1
	}
	resilienceBaseMs = envInt("OPEN_METEO_RETRY_BASE_MS", 200)
	cbThreshold := envInt("OPEN_METEO_CB_THRESHOLD", 5)
	cbCooldownMs := envInt("OPEN_METEO_CB_COOLDOWN_MS", 30000)
...
func DoWithResilience(req *http.Request) (*http.Response, error) {
	...
	if lastErr == nil {
		lastErr = errors.New("open-meteo: request failed after retries")
	}
	return nil, lastErr
}
